...
class WebServer:
    ...
    async def step(self, request):
        ...
        if not self.isPlottingInProgress:
            jData = await request.json()
            self.loop.run_in_executor(self.pool, self.doStep, jData)
            result['status'] = 'Started'
        else:
            result['status'] = 'InProgress'
        return web.json_response(result)

    async def moveTo(self, request):
        logger.debug("moveTo Post")
        result = {}
        if not self.isPlottingInProgress:
            jData = await request.json()
            self.loop.run_in_executor(self.pool, self.doMove, jData)
            result['status'] = 'Started'
        else:
            result['status'] = 'InProgress'
        return web.json_response(result)
